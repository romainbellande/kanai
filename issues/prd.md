...
- 2026-06-02: Added owner-only backend column reorder through `PUT /projects/{project_id}/columns/reorder`. Column reorder now accepts the complete ordered column-ID list, rejects duplicate, missing, or foreign column IDs before writing positions, rewrites contiguous integer positions atomically in the service, returns the persisted ordered columns, and remains hidden behind owner-only access. Remaining owner-only column management includes delete; broader task `column_id`, startup reset, OpenAPI, and frontend work remains.
- 2026-06-02: Added owner-only backend column deletion through `DELETE /projects/{project_id}/columns/{column_id}`. Column deletion now rejects non-owner access, blocks deleting the final remaining column, deletes an empty selected column, and normalizes remaining positions after persistence. Remaining persisted-column work includes task `column_id` migration with non-empty column protection, startup reset simplification, OpenAPI regeneration, and frontend column flows.
- 2026-06-02: Added the first backend task `column_id` migration slice. Tasks now have a required persisted `column_id`, create defaults to the first project column when omitted, create/update reject columns outside the project, update keeps the legacy `status` mirror aligned to the selected column name, task responses expose `column_id`, and project column deletion now rejects non-empty columns. Remaining persisted-column work includes removing legacy task status from the API/move workflow, startup reset simplification, OpenAPI regeneration, and frontend column flows.
- 2026-06-02: Simplified startup database initialization for development schema changes. When startup initialization is enabled, `create_db_and_tables` now drops and recreates SQLModel metadata tables so obsolete local schemas are replaced by the current model shape; non-init environments still skip startup schema changes, and the legacy task-rank repair path has been removed. Remaining persisted-column work includes removing legacy task status from the API/move workflow, OpenAPI regeneration, and frontend column flows.
- 2026-06-02: Removed legacy task `status` from the backend task API contract and board move workflow. Task create/update/destination payloads now reject `status`, task read payloads expose `column_id` without `status`, task movement targets persisted `column_id`, and task repository ordering/filtering uses columns instead of status. Remaining persisted-column work includes OpenAPI regeneration and frontend column flows.
- 2026-06-02: Improved frontend OpenAPI generation infrastructure so `bun --bun run gen:openapi:client` first exports the schema from the local backend app into an ignored `.generated/openapi.json`, removing the need for a separately running API server. Full client regeneration remains blocked in this environment because the configured OpenAPI generator requires `java`, and Docker is installed but its daemon is not running. Remaining persisted-column work still includes completing OpenAPI client regeneration and frontend column flows.
- 2026-06-02: Added a manual frontend project-column API/cache boundary while generated client regeneration remains blocked. The projects client now exposes typed list/create/update/reorder/delete wrappers for nested project column endpoints, stable column query keys/options, and `useKanaiApi().projectColumns` facade methods with cache invalidation. Remaining frontend work includes wiring the board, task create, and task detail flows to persisted columns and completing generated client regeneration once Java or Docker is available.
- 2026-06-02: Wired the project board read/move workflow to persisted project columns. `useProjectTaskBoard` now loads project columns through the facade, groups tasks by `column_id`, optimistically moves tasks with `column_id` and rank, and the board page renders persisted column names with a retryable column-load error instead of hardcoded fallback columns. The task API wrapper now maps task `column_id` manually while OpenAPI regeneration remains blocked. Remaining frontend work includes owner inline column management plus task create/detail column selectors, and generated client regeneration once Java or Docker is available.
